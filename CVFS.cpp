...
#include<string.h>      //inbuilt functions of string
#include<unistd.h>      //macro sathi
#include<iostream>     //cpp code
...
#define REGULAR 1           // 2 types of file 
#define SPECIAL 1
...
{
 	char FileName[50];          //actual file name directory madhe asta  
 	int InodeNumber;
 	int FileSize;
 	int FileActualSize;
 	int FileType;
 	char *Buffer;
 	int LinkCount;           //shortcut create kelyavr count vadhnar
 	int ReferenceCount;        //file jevdhya open keli tr count vadnar           //how many refernces are refering to that file
...
{
	int readoffset;           //kuthun data read kartay tyacha offset
	int writeoffset;           //data kuth lihitay tyacha offset
	int count;
	int mode;   //1  2  3
	PINODE ptrinode;
...
PINODE head=NULL;        //global variable  to access  DILB in functions
...
void man(char *name)
{
	...
	else if(strcmp(name,"open")==0)
	{
		...
		printf("Usage : open File_name model\n");
	}
...
void CreateDILB()
{
	...
	printf("DILB created successfuly\n");
}
...
int CreateFile(char *name, int permission)
{
	...
	if((name==NULL) ||  (permission==0) ||  (permission>3))      //incorrect parameter
		return -1;
		
	if(SUPERBLOCKobj.FreeInode==0)         //no inodes left
		return -2;
	
	(SUPERBLOCKobj.FreeInode)--;
	
	if(Get_Inode(name)!=NULL)           //file name already exists
		return -3;
		
	while(temp!=NULL)
	{
		if(temp->FileType==0)       //filetype==0 means inode is empty
			break;
		temp=temp->next;
	}
	while(i<MAXINODE)
	...
	UFDTArr[i].ptrfiletable->ptrinode->ReferenceCount=1;      //Reference count means kiti vela tya file la open kelay
...
int main()
{
	...
	char  command[4][80],str[80],arr[1024];           //80-no of character on terminal horizontally
	...
	while(1)
	{
		...
		strcpy(str,"");          //or use memset()
		
		printf("\nMarvellous VFS : >  ");
		fgets(str,80,stdin);   // getline()    take input from user   
		count=sscanf(str,"%s %s %s %s",command[0],command[1],command[2],command[3]);      //strtok()       
