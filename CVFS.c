...
int CreateFile(char *name, int permission)
{
	...
	UFDTArr[i].ptrfiletable->ptrinode->LinkCount=MAXFILESIZE;
...
int ReadFile(int fd, char *arr, int isize)
{
	...
	if(UFDTArr[fd].ptrfiletable->ptrinode->mode!=READ  &&  UFDTArr[fd].ptrfiletable->ptrinode->mode!=READ+WRITE)      return -2;
	...
	read_size=(UFDTArr[fd].ptrfiletable->FileActualSize)-(UFDTArr[fd].ptrfiletable->readoffset)   
...
int LseekFile(int fd, int size, int from)
{
	...
	if(UFDTArr[fd].ptrfiletable->mode==READ) ||  (UFDTArr[fd].ptrfiletable->mode==READ+WRITE))
	{
		if(from==CURRENT)
		{
			...
			if(((UFDTArr[fd].ptrfiletable->readoffset)_size)<0)	return -1;
...
int truncate_file(char* name)
{
	...
	if(fd==-1)	return -1;
	mamset(UFDTArr[fd].ptrfiletable->ptrinode->Buffer,0,1024);
	UFDTArr[fd].ptrfiletable->readoffset=0;
	UFDTArr[fd].ptrfiletable->writeoffset=0;
